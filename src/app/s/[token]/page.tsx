    ...
    <div className="container">
      ...
      <main>
        <div className="card">
          ...
          <p>
            <span className="badge">{data.report.playerClass} {data.report.spec}</span>{" "}
            <span className={`badge ${data.report.result ? "badge-ok" : "badge-err"}`}>
              {data.report.result ? "限时成功" : "未限时完成"}
            </span>{" "}
            <span className="badge">分享于 {new Date(data.report.createdAt).toLocaleDateString("zh-CN")}</span>
          </p>
