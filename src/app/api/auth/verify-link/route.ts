...
/**
 * POST /api/auth/verify-link —— 邮箱魔法链接登录（FR-7 增强）。
 * 生产 Supabase 对新用户首次登录发送 sign-in 链接，用户点击后带
 * ?token_hash=...&type=email（老形式 ?code=...）回到站点；本接口用
 * verifyOtp({ type: "email", token_hash }) 建立会话（cookie 由现有
 * createServerClient 桥接），成功后前端跳转首页 / 历史。
 */
export async function POST(req: NextRequest) {
  ...
  const { tokenHash, email } = parsed.data;
  ...
  const result = await auth.verifyLink(tokenHash, email);
  if (!result.ok) {
