...
describe("POST /api/auth/verify-link（魔法链接登录）", () => {
  it("成功 → 200 ok，透传用户邮箱，verifyLink 收到 tokenHash + email", async () => {
    verifyLink.mockResolvedValue({ ok: true, user: { id: "u1", email: "a@b.com" } });

    const res = await POST(fakeReq({ tokenHash: "tok", email: "a@b.com" }));

    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.ok).toBe(true);
    expect(body.email).toBe("a@b.com");
    expect(verifyLink).toHaveBeenCalledWith("tok", "a@b.com");
  });

  it("无 email 也可成功（token_hash 优先）", async () => {
    verifyLink.mockResolvedValue({ ok: true, user: { id: "u2", email: "b@c.com" } });

    const res = await POST(fakeReq({ tokenHash: "tok2" }));

    expect(res.status).toBe(200);
    expect(verifyLink).toHaveBeenCalledWith("tok2", undefined);
  });
