...
export const CHAPTER_SPECIFIC_INSTRUCTIONS: Record<number, string> = {
  ...
  5: `【章节5】战术意图识别：对每个"可疑操作"先检查是否存在战术合理性（对齐易伤/CD 规划/留资源/路线安排/职业打法技巧等），再下结论。
判定规则（三档）：
- 第一档"正确决策"：存在合理意图（如：无爆发时喝爆发药水是为了对齐 300 秒后的 BOSS 易伤；预留爆发对齐易伤窗口；提前开减伤覆盖高伤害机制；故意不断控以攒控链）→ 归入本章并解释"为什么是正确决策"。
