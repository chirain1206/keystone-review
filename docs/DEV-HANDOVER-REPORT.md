...
| T18 | 初始知识库内容：**火焰法师（12 条）/ 兽王猎人（12 条）/ 防护战士（12 条）**，均为至暗之夜 12.1 大秘境 S2 要点摘要（意图模式/爆发规划/资源管理/副本机制/补丁变动），frontmatter 带 patch=12.1 + 出处链接（NGA/Wowhead/B 站等，取自调研报告） | `ingest.test.ts` 内容合规用例：3 个文件 ≥10 条/文件、patch=12.1、source_url http(s)、单条 ≤1200 字符（要点摘要非整篇搬运）；出处链接来源见 docs/rag-community-knowledge-feasibility.md（QA 可打开验证） |
