...
### 核心决策：路线指纹与相似度对比（ADR-003，用户新需求，批次 7）

- **背景**：调研确认 WCL 与第三方均无"两条 log 路线对比/diff"能力；且 WCL API 的 pull 数据**已把 chain 接波合并**、无每怪进战斗时间戳——波次失真的唯一解法是自解析原始 log。两个用途（用户提出）：① 选参考目标时匹配路线与阵容类型（法刀大波 vs 菜刀短平快，强行对比无意义）；② 挖掘时"同路线但波次边界不同"的 log 不丢弃（不减数据源）。
- **决策（全部自研核心算法）**：
  1. **战术波还原（chain 归一化）**：解析器扩展——原始 log 毫秒级事件中，上一批怪进入残血/濒死阶段时新一批怪首次进入战斗 = chain 接波；产出"战术波"序列（每波怪物清单+起止时间），作为路线指纹的基础单元；
  2. **路线指纹**：波序列，每波 = 怪物签名（NPC 标识/类型+数量，模糊匹配容忍同类怪数量小差异）+ 相对时间锚（进本/关键 BOSS 节点归一化）；
  3. **相似度比对**：波序列对齐（类似序列 diff；怪物构成模糊匹配 + 顺序小差异与合波方式差异容差）→ 相似度 0–1 + 差异清单；
  4. **阵容类型分类**：启发式规则（阵容职业构成 + 每 pull 平均怪数/进度风格）→ 法刀 / 菜刀 / 混合；
  5. **落地**：参考目标推荐排序（相似度+阵容）+ T20 挖掘工具同路线分组。
- **数据源**：自解析原始 log 为主；WCL v2 API（ReportDungeonPull/ReportDungeonPullNPC/ReportFight）为辅助——字段名在开发时对照官方文档核实（调研员因网络限制未逐字核对）。
- **放弃项**：依赖 WCL pull 分段做路线（波次失真）；MDT 规划路线对比（远期可选，用现成 mdt-compression 解析）；全自动路线聚类（远期）。
