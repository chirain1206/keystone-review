...
- **内容与更新**：调研确认当前为《至暗之夜》补丁 12.1、大秘境第 2 赛季；来源优先级 **NGA 各职业精华帖 > Wowhead 中/英文指南 > B站（头部攻略 up）> Icy Veins > 17173/网易大神**。所有站点无公开 API、ToS 禁止爬取 → **人工整理 + 定期更新**是唯一合规路径：调研子 Agent 起草 → 主 Agent 审核 → 入库，更新周期 ≤1 周（FR-11）；**只存要点摘要与出处链接，不整篇搬运**（版权合规）。详细调研见 docs/rag-community-knowledge-feasibility.md。
